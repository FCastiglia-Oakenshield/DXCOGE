...
Public Module LibroCespPDF
    Dim PathPrg, Dest, FtFm, Ftmp, Concat, Stampa, QualeLp, NomeFile, QualeTipo, DysEse, DysIn, DysFi, DysUl As String
    ...
    Dim MaxRighe As Int16 = 70
    Dim NumRighe, LL As Int16
    ...
    Public Sub StampaLibro(ByVal Anno As Int16, ByVal DaCateg As Int16, ByVal ACateg As Int16, ByVal AnniPrecedenti As Boolean, ByVal Tutti As Boolean, ByVal SuBol As Boolean, ByVal Intesta As String, ByVal n1 As Int16, ByVal n2 As Int32, ByVal AZI As String, ByRef FILESTAMPA As String, ByVal DataMin As Date, ByVal DataMax As Date, ByVal Periodo As String)
        TotVal = 0
        TotCos = 0
        TotGQuota = 0
        TotGFondo = 0
        ...
        IT(6) = Intesta
        Inizio = TimeString
        DysEse = " Esercizio " & DataMin.ToShortDateString & " - " & DataMax.ToShortDateString
        DysIn = Format(DataMin, "dd/MM")
        DysFi = Format(DataMax, "dd/MM")
        DysUl = DataMax.ToShortDateString
        ...
        ''''  Dest = PathTmp & "Print."
        VerifyDir()
        Tdoc = "LIBROCESPITI.Pdf"
        NomeFile = PathTmp & Tdoc
        FILESTAMPA = NomeFile
        settaggi(0)
        Pagine = n2
        NumRighe = 0
    ...
    Private Sub SezTitolo(ByVal Rw As DataRow)
        ...
        Str = DysEse.PadRight(52, " ") & Rw("CespCat") & " " & Rw("CspDesc").ToString.PadRight(43) ' & " Pag. N. " & Pagine.ToString.PadLeft(3)
        ...
        Else
            ...
            PRT(8) = IT(0).PadRight(55, " ") & IT(6).PadRight(LL, " ") & "Pagina N. " & AnnoLp & barra & Pagine.ToString.PadLeft(6, " ")
    ...
    Private Sub SezQuo(ByVal Rw As Object)
        ...
        Dim Str As String
        Str = "    |    |" & Anno & "|    |      | |V.Bene - " & DysIn & "|" & CoAmmIni & "|" & FondoIni & "|Fondo -  " & DysIn & "|     |           |           |           |           |"
        PrintLineX(Str)
        If AnnoRif = Rw("QuoAnno") Then
    ...
    Private Sub SezQuoTot(ByVal Rw As Object, ByVal Fine As Boolean)
        ...
        Str = "    |    |    |    |      | |-------------------------------------------------|     |           |           |           |           |"
        PrintLineX(Str)
        Str = "    |    |    |    |      | |TOTALI - " & DysFi & "|" & Totale1 & "|" & Totale2 & "|CxA|" & CoAmm & "|" & Ali & "|" & Quota & "|" & Fondo & "|" & Residuo & "|" & NoDetra & "|"
        PrintLineX(Str)
    ...
    Private Sub TotaliGenerali()
        ...
        Str = "                                             ---------------------------------|     |           |           |           |           |"
          PrintLineX(Str)
        Str = "A L  " & DysUl & "                             |" & TVal & "|" & TCos & "|     |" & TQuo & "|" & TFon & "|" & TRes & "|" & TND & "|"
        PrintLineX(Str)
        Str = "                                             ---------------------------------------------------------------------------------------"
        PrintLineX(Str)
    ...
    Private Sub PrintLineX(ByVal Line As String)
        ...
        If NumRighe = MaxRighe Then
            Dim Str As String
            Str = "".PadLeft(TotCol, "-")
            myPhrase = New Phrase(Str & Microsoft.VisualBasic.Chr(10) & "", New Font(Font.COURIER, 6.5))
            doc.Add(myPhrase)
            doc.NewPage()
            NumRighe = 1
            ''   If Trim(ComandoInizializza) > "" Then PrintLine(1, ComandoInizializza)
            SezTitolo(RwCat)
        End If
        myPhrase = New Phrase(Line & Microsoft.VisualBasic.Chr(10) & "", New Font(Font.COURIER, 6.5))
        doc.Add(myPhrase)
    End Sub
