...
Public Class LDPSchedLDP
    ...
    Private Sub LDPSchedLDP_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        If sw = 0 Then
            GroupControl1.Text = "MOVIMENTO dei CONTI"
            L1 = "PrkAAmmgg" 'Procedura NO COMPETENZA
            L1x = "[Data Operazione]"
            T2 = ""
            DateEdit5.EditValue = Today
            Apertura()
            sw = 1
        End If
    ...
    Sub Esercizi(ByVal x As Int16)
        DateEdit1.Properties.MaxValue = "31/12/2050" ' reset campi per ricalcolare limiti
        DateEdit1.Properties.MinValue = "01/01/1900"
        DateEdit4.Properties.MaxValue = "31/12/2050"
        DateEdit4.Properties.MinValue = "01/01/1900"
        DateEdit1.Properties.MaxValue = EseDal(x).AddMinutes(1)
        DateEdit1.Properties.MinValue = EseDal(x)
        DateEdit1.EditValue = EseDal(x)
        DateEdit4.Properties.MaxValue = EseAl(x).AddMinutes(1)
        DateEdit4.Properties.MinValue = EseAl(x)
        DateEdit4.EditValue = EseAl(x)
        DateEdit5.Properties.MaxValue = "31/12/2050"
        DateEdit5.Properties.MinValue = "01/01/1900"
        DateEdit5.EditValue = DateEdit4.EditValue
        DateEdit5.Properties.MaxValue = DateEdit4.Properties.MaxValue
        DateEdit5.Properties.MinValue = DateEdit1.Properties.MinValue
        If DateEdit5.EditValue > Today Then DateEdit5.EditValue = Today
    ...
    Sub Stampa()
        ...
        '    DateEdit5.EditValue = CDate(DataMin)
        ...
        Dim Order As String = " order by LdpSigla, MCCCogConto, " & L1 & ""
        Dim Str As String = ""
        ...
        End If
        If LINDO = True Then
            DateEdit5.EditValue = Nothing : DateEdit4.EditValue = Nothing
        End If
    End Sub
    Sub PreparoFormula()
        ...
        '    dd1 = "DALC" : dd2 = "ALC"
        'Else
        dd1 = CDate(DateEdit1.EditValue).ToShortDateString : dd2 = CDate(DateEdit4.EditValue).ToShortDateString
        'End If
        For JJ As Int16 = 1 To DsAgD.Rows.Count
            Rw = DsAgD.Rows(JJ - 1)
            Formula = Formula & "MCCCOGLDP = " & Rw("LdpRif") & " OR "
            ForExp = ForExp & "[L D P]= '" & Rw("LdpSigla") & "' OR "
    ...
    Private Sub DateTimePicker1_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DateEdit5.EditValueChanged
        Dim Mese, Giorno, Anno As String
        Mese = CDate(DateEdit5.EditValue).Month
        Anno = CDate(DateEdit5.EditValue).Year
        Giorno = Date.DaysInMonth(Anno, Mese)
        If CDate(DateEdit5.EditValue).ToShortDateString = CDate(Giorno & "/" & Mese & "/" & Anno).ToShortDateString Then
