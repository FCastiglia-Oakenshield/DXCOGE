...
Public Class DxRiparto
    ...
    Private Sub DxRiparto_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown, ButtonF5.Click
        Pulizia(0)
        If sw = 0 Then
            'Cursor.Current = Cursors.WaitCursor
            'EsegueSql("XCRONRIP", cnDb)
            'Cursor.Current = Cursors.Default
            Apertura() : CommesseERepertorio() : RadioGroup1.SelectedIndex = 0
            sw = 1
        End If
    ...
    End Sub
    Sub CommesseERepertorio()
        'Carico Commesse Aperte
        Dim ii(0) As Integer
        Dim i As Int16 = 0
        Dim n As Int16 = 0
        ...
        dataRd = Cmd.ExecuteReader
        While dataRd.Read
            nx = New DevExpress.XtraEditors.Controls.ImageComboBoxItem(dataRd.Item("TcmSigla"))
            RepositoryItemComboBox3.Items.Add(nx)
            ReDim Preserve Commesse(i)
            Commesse(i) = dataRd.Item("TcmRif")
            i += 1
        End While
        dataRd.Close()
        'Repertorio
    ...
    Sub PopolaRegime(ByVal i As Int16)
        ...
        Xreg = DsReg.Tables(Ri).Rows.Count - 1 '' SELEZIONO IVA ACQUISTI
...
DopoLet:
    ...
    Sub DaRipartire()
        ...
        If TextEdit3.EditValue > 0 Then ButtonHelp.Enabled = True Else ButtonHelp.Enabled = False
    ...
    Sub CaricaTotali()
        ...
        For y = 1 To DsMcc.Rows.Count
            ...
            For q = 1 To DsCpt.Rows.Count
                RwCpt = DsCpt.Rows(q - 1)
                If RwMcc("MCCCOGCONTO") = RwCpt("COGCONTO") And RwMcc("DAREAVERE") = RwCpt("DAREAVERE") And RwMcc("MCCCogDet") > 0 Then
                    RwCpt("RIPARTITO") = RwCpt("RIPARTITO") + RwMcc("MCCIMPORTO")
                    RwCpt("RESIDUO") = RwCpt("IMPORTO") - RwCpt("RIPARTITO")
                ElseIf RwMcc("MCCCOGCONTO") = RwCpt("COGCONTO") And RwMcc("DAREAVERE") = RwCpt("DAREAVERE") And RwMcc("MCCCogDet") = 0 Then
                    RwCpt("RIPARTITO") = 0
            ...
            If RwCpt("RESIDUO") <> 0 Then
                For i = 1 To DsMcc.Rows.Count
                    ...
                    If DsMcc.Rows(i - 1).Item("MCCCOGCONTO") = RwCpt("COGCONTO") Then
                        ...
                        Next
                        RwMcc("MCCPROG") = 0 : RwMcc("MCCIMPORTO") = RwCpt("RESIDUO")
                ...
                Next
                RwMcc("MCCPROG") = 0 : RwMcc("MCCIMPORTO") = RwCpt("RESIDUO") : RwMcc("MCCCOGCONTO") = RwCpt("COGCONTO")
                DsMcc.Rows.Add(RwMcc)
                DsMcc.AcceptChanges()
                CheckButton1.Visible = False
                GridColumn19.OptionsColumn.ReadOnly = False : GridColumn19.OptionsColumn.AllowEdit = True
...
#Region "GESTIONE GRID RIPARTIZIONE"
    ...
    Private Sub GridView4_RowUpdated(sender As Object, e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles GridView4.RowUpdated
        ...
        If RwV("TCMSIGLA") Is DBNull.Value Then RwV("TCMSIGLA") = ""
    ...
    Sub AssegnaRepertorio()
        ...
        For I As Int16 = 1 To RepositoryItemComboBox3.Items.Count
            If RwMcc("TCMSIGLA") = RepositoryItemComboBox3.Items(I - 1) Then RwMcc("MccCogLdp") = Commesse(I - 1) : Exit For
        Next
    End Sub
#End Region
    ...
    Function Controlla(n As Int16) As Boolean
        ...
        For I = 1 To DsMcc.Rows.Count
            ...
            If RwMcc("VOCE").ToString.Length = 0 And RwMcc("MCCIMPORTO") <> 0 And n = 0 Then Mail &= "Riga " & I & " MANCA REPERTORIO" & Chr(10)
            If RwMcc("TCMSIGLA").ToString.Length = 0 And RwMcc("MCCIMPORTO") <> 0 Then Mail &= "Riga " & I & " MANCA COMMESSA" & Chr(10)
        Next
        If Mail > "" Then
            MoltoCritico(Mail)
            Return False
...
& " values(@MCCID,@MCCPROG,@MCCPrkAammgg,@MCCPrkId,@MCCPrkProg,@MCCPrkDa,@MCCCogLdp,@MCCCogCdc,@MCCCogRep,@MCCCogDet,@MCCCogConto,@MCCIMPORTO)"
        ...
        For I As Int16 = 1 To DsMcc.Rows.Count
            ...
            If RwCha("MCCIMPORTO") <> 0 Then
                ...
                mp9.Value = CInt(Mid(RwCha("VOCE").ToString, 3, 2))
                mp9a.Value = CInt(Mid(RwCha("VOCE").ToString, 6, 2))
